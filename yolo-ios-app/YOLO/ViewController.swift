...
class ViewController: UIViewController {
    ...
    var sendFrameTimer: Timer?
    var lastCapturedImage: UIImage?
    
    //let speechHandler = SpeechHandler.shared
    var latestResults: [VNRecognizedObjectObservation]? = nil
    ...
    override func viewDidLoad() {
        //setupSpeechHandler()
        super.viewDidLoad()
        setupMapTestingButton()
        setupSpeechInteractionButton()
        setupTextDetectionTestingButton()
        slider.value = 30
        setLabels()
        ...
        startVideo()
        // setModel()
        startTextDetectionTimer()
        
    }
    
    deinit {
        sendFrameTimer?.invalidate()
    }
    ...
    @IBAction func indexChanged(_ sender: Any) {
        ...
        case 4:
            self.labelName.text = "dense"
            //mlModel = try! yolov8x(configuration: .init()).model
            mlModel = try! yolov8s_worldv2(configuration: .init()).model
        default:
            break
        }
        setModel()
    ...
    func setupSpeechInteractionButton() {
        let mapButton = UIBarButtonItem(
            image: UIImage(systemName: "mic.circle"),
            style: .plain,
            target: self,
            action: #selector(speechInteractionButtonTapped)
        )
        var toolbarItems = self.toolBar.items ?? []
        toolbarItems.append(mapButton)
        self.toolBar.setItems(toolbarItems, animated: false)
        
    }
    
    @objc func speechInteractionButtonTapped() {
        let speechView = UIHostingController(rootView: SpeechToTextView())
        self.present(speechView, animated: true)
    }
    
    func setupMapTestingButton() {
        let mapButton = UIBarButtonItem(
    ...
    @objc func mapButtonTapped() {
        let mappingView = UIHostingController(rootView: MapView())
        self.present(mappingView, animated: true)
    }
    
    func setupTextDetectionTestingButton() {
        let ocrButton = UIBarButtonItem(
            image: UIImage(systemName: "text.viewfinder"),
            style: .plain,
            target: self,
            action: #selector(textDetectionButtonTapped)
        )
        var toolbarItems = self.toolBar.items ?? []
        toolbarItems.append(ocrButton)
        self.toolBar.setItems(toolbarItems, animated: false)
        
    }
    
    @objc func textDetectionButtonTapped() {
        let textDetectView = UIHostingController(rootView: TextDetectionCameraView())
        self.present(textDetectView, animated: true)
    }
    ...
    func predict(sampleBuffer: CMSampleBuffer) {
        if currentBuffer == nil, let pixelBuffer = CMSampleBufferGetImageBuffer(sampleBuffer) {
            ...
            // Create UIImage from pixelBuffer
    ...
    func processObservations(for request: VNRequest, error: Error?) {
        DispatchQueue.main.async {
            ...
            if let results = request.results as? [VNCoreMLFeatureValueObservation] {
                ...
                guard let featureValue = results.first?.featureValue.multiArrayValue else {return}
                let predictionCount = featureValue.shape[1].intValue // 25200
                let attributesPerPrediction = featureValue.shape[2].intValue // 6
                
                // Ensure the structure is valid
                //guard attributesPerPrediction == 6 else {
                   // print("Unexpected attribute count per prediction.")
                   // return
                //}
                
                // Access MultiArray data
                let pointer = featureValue.dataPointer.bindMemory(to: Float32.self, capacity: featureValue.count)
                var boundingBoxes: [(xMin: Float32, yMin: Float32, normalizedWidth: Float32, normalizedHeight: Float32, confidence: Float32, classLabel: Float32)] = []
                var xMIN: Float = 9999
                for i in 0..<predictionCount {
                    let startIndex = i * attributesPerPrediction
                    var xMin = pointer[startIndex] / 640.0
                    var yMin = (384.0 - pointer[startIndex + 1]) / 384.0
                    let width = pointer[startIndex + 2] / 640.0
                    let height = pointer[startIndex + 3] / 384.0
                    let confidence = pointer[startIndex + 4]
                    let classLabel = pointer[startIndex + 5]
                    xMin -= width / 2
                    yMin -= height / 2
                    print("here \(pointer[startIndex]) \(pointer[startIndex + 1]) \(pointer[startIndex] + 2) \(pointer[startIndex + 3])")
                    // Optionally filter results by confidence threshold
                    if confidence > 0.5 && classLabel == 1  {
                        
                        
                        xMIN = min(xMin, xMIN)
                        boundingBoxes.append((xMin, yMin, width, height, confidence, classLabel))
                    }
                }
                self.showDense(boxes: boundingBoxes)
                //print("the count is \(counter)")
                //print("the xMIN \(xMIN)")
                //SeenObjectService.shared.reportDetectedResults(results: results)
                //self.show(predictions: results)
            } else {
                //print(type(of: request.results?.first))
    ...
    func showDense(boxes: [(xMin: Float32, yMin: Float32, normalizedWidth: Float32, normalizedHeight: Float32, confidence: Float32, classLabel: Float32)]) {
        ...
        for i in 0..<boundingBoxViews.count {
            ...
            if i < boxes.count {
                let box = boxes[i]
                var rect = CGRect(origin: .init(x: CGFloat(box.xMin), y: CGFloat(box.yMin)), size: .init(
                    width: CGFloat(box.normalizedWidth),
                    height: CGFloat(box.normalizedHeight)
                ))
    ...
    func startTextDetectionTimer() {
        //sendFrameTimer = Timer.scheduledTimer(timeInterval: 3.0, target: self, selector: #selector(sendCurrentFrame), userInfo: nil, repeats: true)
        let textDetectionTimer = Timer.scheduledTimer(withTimeInterval: 5, repeats: true, block: { timer in
            
            Task {
                let img = await self.lastCapturedImage
                TextDetectManager.shared.detectText(img: img)
            }
