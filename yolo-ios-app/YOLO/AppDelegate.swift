...
class AppDelegate: UIResponder, UIApplicationDelegate {
  ...
  func application(
    ...
    func application(_ app: UIApplication, open url: URL, options: [UIApplication.OpenURLOptionsKey : Any] = [:]) -> Bool {
        // Check if this is a callback from Google Maps
        if url.scheme == "sypppGithubIoYoloNavigationCompleted" {
            // Handle the return from Google Maps
            NotificationCenter.default.post(name: Notification.Name("GoogleMapsNavigationCompleted"), object: nil)
            print("sypppppp navigation complet!!!!!!!!!!!!")
            return true
        }
        return false
    }
